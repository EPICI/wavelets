	...
	public static void drawButton(Graphics2D graphics,int x,int y,int width,int height,String text,BufferedImage icon,Font font,Color outline,Color innerFillBase,boolean innerFillSolid,Color textFill,Color imageTint,double xalign,double yalign,double imageScale,int imageDirection){
		...
		if(drawText){
			...
			if(drawImage){// Text and image
				...
				boolean vertical = htotalWidth>awidth || vtotalHeight>aheight && htotalWidth*aheight > vtotalHeight*awidth;
				...
				if(vertical){// Draw image above/below text
					double scale = Math.pow(Math.min(awidth/vtotalWidth,aheight/vtotalHeight),imageScale);
					...
					if((imageDirection&2)!=0){// Draw image above text
						double xoffset = BUTTON_CORNER_RADIUS+xalign*(awidth-dimageWidth);
						double yoffset = BUTTON_CORNER_RADIUS+yalign*(aheight-dtotalHeight);
						...
						for(int i=0;i<textCount;i++){
							graphics.drawString(textParts[i], (int)(BUTTON_CORNER_RADIUS+xalign*(awidth-textWidths[i]*scale)), (int)yoffset);
							yoffset+=dlineHeight;
						}
					...
					double scale = Math.pow(Math.min(awidth/htotalWidth,aheight/htotalHeight),imageScale);
					font=font.deriveFont((float)(scale*font.getSize()));
					double dimageWidth = imageWidth*scale;
					double dimageHeight = imageHeight*scale;
					double dlineHeight = lineHeight*scale;
					...
					if((imageDirection&1)!=0){// Draw image to left of text
						double xoffset = BUTTON_CORNER_RADIUS+xalign*(awidth-dtotalWidth);
						double yoffset = BUTTON_CORNER_RADIUS+yalign*(aheight-dimageHeight);
						graphics.drawImage(icon, (int)xoffset, (int)yoffset, (int)(xoffset+dimageWidth), (int)(yoffset+dimageHeight), 0, 0, imageWidth, imageHeight, null);
						xoffset += dimageWidth;
				...
				double scale = Math.pow(Math.min(awidth/textWidth,aheight/textHeight),imageScale);
				font=font.deriveFont((float)(scale*font.getSize()));
				double dlineHeight = lineHeight*scale;
				double dtextHeight = textHeight*scale;
			...
			int imageWidth = icon.getWidth(), imageHeight = icon.getHeight();
			double scale = Math.pow(Math.min(awidth/imageWidth, aheight/imageHeight), imageScale);
			double dimageWidth = imageWidth*scale;
			double dimageHeight = imageHeight*scale;
			if(imageTint!=null&&!imageTint.equals(Color.WHITE)){
				LookupOp lut = lutMultiply(imageTint);
			...
			double xoffset = BUTTON_CORNER_RADIUS+(awidth-dimageWidth)*xalign;
			double yoffset = BUTTON_CORNER_RADIUS+(aheight-dimageHeight)*yalign;
			graphics.drawImage(icon, (int)xoffset, (int)yoffset, (int)(xoffset+dimageWidth), (int)(yoffset+dimageHeight), 0, 0, imageWidth, imageHeight, null);
		}
		// Outline
